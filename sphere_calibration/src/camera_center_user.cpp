...
bool Camera::run(int argc, char** argv){
    ...
    float RatioSamples = 0.15;   // if the ratio is too small, small ellipse will not be detected
    ...
    cv::Point2f image_center(focal_length[2], focal_length[3]); // cx & cy
    ...
    for (int k = 1; k < folder_count + 1; k++) {
        ...
        while (break_iter <= 5) {
            for (int i = 0; i < filenames.size(); i++) {
                ...
                // Check if mask image has multiple masks
                cv::Mat labels;
                int num_clusters = cv::connectedComponents(edges, labels);
                if (num_clusters - 1 != 1) {
            ...
            if (!ellipses.empty()) break; // found ellipse
            if (break_iter == 5) {
        ...
        // Visualize the detected ellipse and filter out the outliers
        cv::Mat raw_image = cv::imread(input_path + "/" + std::to_string(k) + "/image.png");
        std::string window_name = "Scene " + std::to_string(k);
        picked_point = cv::Point(-1, -1);
        int idx = -1;

        if(ellipses.size() == 1) { // if there is one ellipse detected
            for (const auto& ellipse : ellipses){
                cv::ellipse(raw_image, ellipse, cv::Scalar(0, 0, 255), 2);
            }
            std::cout << "   - There is one target(red) in this scene." << std::endl;
            idx = 0;
            cv::imshow(window_name, raw_image);
            cv::setMouseCallback(window_name, mouse_callback, this);
            std::cout << "   - Press any key but 'q' to continue. " << std::endl;
            std::cout << "   - If none of ellipses are valid, press 'q'.\n" << std::endl;

            int key = cv::waitKey(0);
            if (key == 'q') {
                std::cout<<"   - Detected ellipse is invalid.\n"<<std::endl;
                cv::imwrite(input_path + "/" + std::to_string(k) + "/image_output_invalid.png", raw_image);
                cv::destroyWindow(window_name);
                config_ellipse_center["Camera"] = {-1, -1};
            ...
            cv::imshow(window_name, raw_image);
            cv::setMouseCallback(window_name, mouse_callback, this);
            cv::Mat temp_image = raw_image.clone();
            std::cout << "   - Multiple ellipses are detected." << std::endl;
            std::cout << "   - Select the target ellipse region with left click button. To reset, click right button!" << std::endl;
            std::cout << "   - If none of ellipses are valid, press 'q'" << std::endl;

            while (true) {
                cv::circle(temp_image, picked_point, 3, cv::Scalar(0, 255, 0), cv::FILLED);

                cv::imshow(window_name, temp_image);
                int key = cv::waitKey(0);
                if (key == 'q') {
                    std::cout<<"   - Detected ellipses are invalid.\n"<<std::endl;
                    cv::imwrite(input_path + "/" + std::to_string(k) + "/image_output_invalid.png", raw_image);
                    cv::destroyWindow(window_name);
                    break;
                }
            ...
            for (int i = 0; i < ellipses.size(); ++i) {
                const auto &center = ellipse_centers[i];
                double distance = std::sqrt(
                        (center.x - picked_point.x) * (center.x - picked_point.x) + (center.y - picked_point.y) * (center.y - picked_point.y));
        ...
        // Center Compensation
        std::pair<cv::Point2f, float> result_real = find_real_center(ellipses[idx], fx, image_center);
        ...
        // Save image
        cv::ellipse(raw_image, ellipses[idx], cv::Scalar(0, 255, 0), 2);
        cv::circle(raw_image, result_real.first, 4, cv::Scalar(0, 0, 255), -1);
        cv::drawMarker(raw_image, ellipses[idx].center, cv::MARKER_CROSS);
        cv::imwrite(input_path + "/" + std::to_string(k) + "/image_output.png", raw_image);

        outFile << "Eccentricity error  :  " << errors[0] << std::endl;
        outFile << "Before compensation :  " << ellipse_centers[idx] << std::endl;
...
//        cv::waitKey(0);
    ...
    if (clusters.size() > 5 && a / b < 1.3 &&
             (ellipse.center.x > std::max(ellipse.size.width, ellipse.size.height)) &&
             (ellipse.center.y > std::max(ellipse.size.width, ellipse.size.height))){
        return std::make_tuple(ellipse, sampled_points, reason);
    }
...
//                cv::circle(middle_output, point, 4, cv::Scalar(0, 0, 255), -1);
//            }
//            cv::circle(middle_output, evaluation_point[0], 4, cv::Scalar(255, 0, 0), -1);
...
void Camera::on_mouse(int event, int x, int y, int flags) {
    if (event == cv::EVENT_LBUTTONDOWN) {
        picked_point = cv::Point(x, y);
