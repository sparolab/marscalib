...
//            }
        ...
        if (sphere_cloud2->size() > 40) {
            ...
            // Select the first 40 indices after shuffling
            indices.resize(40);
...
//        std::cout << std::sqrt(std::pow(final_center.x,2) + std::pow(final_center.y,2) + std::pow(final_center.z,2)) <<std::endl;
        ...
        viewer4.addPointCloud<pcl::PointXYZI>(cluster_cloud, "cluster");
        ...
        viewer4.addPointCloud<pcl::PointXYZI>(incid_cloud, "candidates");
